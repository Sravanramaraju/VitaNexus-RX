...
function IndicationAutocomplete({ value, onChange }) {
  ...
  return <div className="relative"><label className="block text-sm font-semibold">Diagnosis / Treatment Indication <span className="text-danger">*</span></label><input className="input mt-1" value={query} onFocus={() => setOpen(true)} onBlur={() => setTimeout(() => { setOpen(false); if (!value) setQuery(""); }, 150)} onChange={(event) => { setQuery(event.target.value); setOpen(true); onChange(null); }} placeholder="Search DrugCentral indications" aria-autocomplete="list" aria-required="true" />{open && query && <div className="absolute z-20 mt-1 max-h-72 w-full overflow-y-auto rounded-lg border border-border bg-card shadow-md dark:border-slate-600 dark:bg-slate-800">{error ? <p className="px-3 py-2 text-xs text-danger">Lookup unavailable: {error}</p> : matches.length ? matches.map((option) => <button type="button" key={option.id} className="block w-full px-3 py-2 text-left text-sm hover:bg-primary/10" onMouseDown={(event) => event.preventDefault()} onClick={() => { onChange(option); setQuery(option.display); setOpen(false); }}><strong>{option.display}</strong><span className="block text-xs text-slate-500">{option.source} · {option.datasetVersion}</span>{option.modelSupported === false ? <span className="mt-1 inline-flex rounded-full bg-danger/10 px-2 py-0.5 text-xs font-semibold text-danger">No ADR Assessment</span> : option.modelSupported === true ? <span className="mt-1 inline-flex rounded-full bg-success/10 px-2 py-0.5 text-xs font-semibold text-success">LightGBM covered</span> : <span className="mt-1 inline-flex rounded-full bg-warning/10 px-2 py-0.5 text-xs font-semibold text-warning">Coverage check unavailable</span>}</button>) : <p className="px-3 py-2 text-xs text-slate-500">No matching dataset indication</p>}</div>}<p className="mt-1 text-xs text-slate-500">All DrugCentral treatment indications can be selected. A red label means LightGBM cannot calculate an ADR assessment until a covered indication is selected.</p>{value && <div className="mt-1 text-xs font-semibold">Selected from {value.source} ({value.datasetVersion}){value.modelSupported === false ? <span className="ml-2 text-danger">No ADR Assessment</span> : value.modelSupported === true ? <span className="ml-2 text-success">LightGBM covered</span> : <span className="ml-2 text-warning">Coverage check unavailable</span>}</div>}</div>
}
...
function CurrentMedicationList({ items = [], onChange }) {
  ...
  const add = () => {
    if (!selected) return setWarning("Select a medicine from the dataset results.");
    ...
    onChange([...items, { drugName, enteredName: resolved.enteredName, normalizedName: resolved.normalizedName, brand: resolved.brand || drugName, generic: resolved.generic || drugName, mappingSource: resolved.mappingSource, mappingVersion: resolved.mappingVersion, modelCoverage: selected.currentMedicationModelSupported, dosage: dosage.trim(), frequency: frequency.trim(), activeStatus: "active" }]);
  ...
  return <div className="space-y-3">
    <div className="relative">
      <input className="input" value={selected ? `${selected.brand} (${selected.genericName})` : query} onChange={(event) => { setSelected(null); setQuery(event.target.value); setWarning(""); }} placeholder="Type a brand or generic name" />
      {query && !selected && <div className="absolute z-20 mt-1 max-h-72 w-full overflow-y-auto rounded-lg border border-border bg-card shadow-md dark:border-slate-600 dark:bg-slate-800">
        {error ? <p className="px-3 py-2 text-xs text-danger">Lookup unavailable: {error}</p> : matches.length ? matches.map((item) => {
          const unsupported = item.currentMedicationModelSupported === false;
          return <button type="button" key={item.id} onClick={() => { setSelected(item); setQuery(""); setWarning(""); }} className="block w-full px-3 py-2 text-left text-sm hover:bg-primary/10">
            <strong>{item.brand}</strong>
            <span className="block text-xs text-slate-500">Generic: {item.genericName} · {item.mappingSource}</span>
            {unsupported
              ...
              : item.currentMedicationModelSupported === true
        ...
        }) : <p className="px-3 py-2 text-xs text-slate-500">No matching Indian brand or generic medicine</p>}
      </div>}
    </div>
    <div className="grid gap-2 sm:grid-cols-3"><input className="input" value={dosage} onChange={(event) => setDosage(event.target.value)} placeholder="Dosage (optional)" /><input className="input" value={frequency} onChange={(event) => setFrequency(event.target.value)} placeholder="Frequency (optional)" /><button type="button" className="btn-secondary" onClick={add}>Add medication</button></div>
    {warning && <p role="alert" className="text-xs font-medium text-warning">{warning}</p>}
    <p className="text-xs text-slate-500">All dataset medicines can be added. A red “No ADR Assessment” label means LightGBM does not contain that medicine in its current-medication vocabulary.</p>
    {items.length > 0 && <div className="space-y-2">{items.map((item, index) => <div key={`${item.drugName || item.brand}-${index}`} className="flex flex-wrap items-center justify-between gap-2 rounded-lg bg-slate-50 p-3 text-sm dark:bg-slate-700/50"><span><strong>Entered name: {item.enteredName || item.brand}</strong> · Generic name: {item.generic || item.drugName}{item.modelCoverage === false && <small className="mt-1 block font-semibold text-danger">No ADR Assessment</small>}<small className="block text-xs text-slate-500">Mapping source: {item.mappingSource || "Indian Medicine Dataset"}</small></span><div className="flex items-center gap-2"><select className="input py-1 text-xs" value={item.activeStatus || "active"} onChange={(event) => onChange(items.map((entry, entryIndex) => entryIndex === index ? { ...entry, activeStatus: event.target.value } : entry))}><option value="active">Active</option><option value="completed">Completed</option><option value="discontinued">Discontinued</option></select><button type="button" className="rounded p-1 text-primary hover:bg-primary/10" onClick={() => setEditingIndex(index)} aria-label={`Edit ${item.drugName || item.brand}`}><Pencil size={15} /></button><button type="button" className="text-xs font-semibold text-danger" onClick={() => onChange(items.filter((_, entryIndex) => entryIndex !== index))}>Remove</button></div></div>)}</div>}
    {editing && <div className="rounded-lg border border-primary/30 p-3"><p className="text-sm font-semibold">Edit {editing.drugName || editing.brand}</p><div className="mt-2 grid gap-2 sm:grid-cols-2"><label className="text-xs font-semibold">Dosage<input className="input mt-1" value={editing.dosage || ""} onChange={(event) => updateMedication({ dosage: event.target.value })} /></label><label className="text-xs font-semibold">Frequency<input className="input mt-1" value={editing.frequency || ""} onChange={(event) => updateMedication({ frequency: event.target.value })} /></label></div><button type="button" className="btn-secondary mt-3" onClick={() => setEditingIndex(null)}>Done</button></div>}
  </div>
}
