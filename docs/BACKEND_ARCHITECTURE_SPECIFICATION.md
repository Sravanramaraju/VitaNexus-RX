...
> **Historical architecture record — do not use as the current runtime contract.** This file contains pre-dataset and prototype analysis retained for audit/history. The authoritative frontend, backend, PostgreSQL, data-import, DDInter/DrugCentral, FAERS ML, API, and operational documentation is [CURRENT_SYSTEM_ARCHITECTURE.md](CURRENT_SYSTEM_ARCHITECTURE.md), verified 2026-08-24. Statements below about static data, browser-local patient data, `DEMONSTRATION_ONLY` results, or absent ADR participation are superseded.
