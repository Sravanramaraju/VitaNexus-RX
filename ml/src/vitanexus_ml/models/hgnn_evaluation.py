...
def multilabel_metrics(targets, probabilities, thresholds=0.5) -> dict:
    ...
    return {
        ...
        **multilabel_decision_metrics(y, p, thresholds),
    }


def probability_metrics(targets, probabilities) -> dict:
    y, p = _arrays(targets, probabilities)
...
def multilabel_decision_metrics(targets, probabilities, thresholds=0.5) -> dict:
    y, p = _arrays(targets, probabilities)
...
def _counts_at_thresholds(labels: np.ndarray, scores: np.ndarray, thresholds: np.ndarray) -> tuple[np.ndarray, ...]:
    # Sweep thresholds on their ordered grid in linear time. This avoids sorting
    # each ADR repeatedly during cross-validation.
    indexes = np.searchsorted(thresholds, scores, side="right") - 1
    indexes = np.clip(indexes, 0, len(thresholds) - 1)
    total_by_bin = np.bincount(indexes, minlength=len(thresholds)).astype(np.int64)
    positive_by_bin = np.bincount(indexes, weights=labels, minlength=len(thresholds)).astype(np.int64)
    predicted = np.cumsum(total_by_bin[::-1])[::-1]
...
def _best_binary_threshold(labels: np.ndarray, scores: np.ndarray, fallback: float) -> ThresholdChoice:
    ...
    tp, fp, fn, _ = _counts_at_thresholds(labels, scores, thresholds)
    precision = np.divide(tp, tp + fp, out=np.zeros_like(tp, dtype=float), where=(tp + fp) > 0)
    recall = np.divide(tp, tp + fn, out=np.zeros_like(tp, dtype=float), where=(tp + fn) > 0)
    f1 = np.divide(2 * precision * recall, precision + recall, out=np.zeros_like(precision), where=(precision + recall) > 0)
    best = max(range(len(thresholds)), key=lambda index: (f1[index], precision[index], thresholds[index]))
    return ThresholdChoice(
        float(thresholds[best]), float(f1[best]), float(precision[best]), float(recall[best])
    )
