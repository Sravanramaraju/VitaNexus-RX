...
export default function AdverseRiskAssessment() {
  ...
  const [state, setState] = useState(visit?.adrPrediction ? (visit.adrPrediction.status === "OUT_OF_VOCABULARY" ? "NEEDS_INPUT" : ["ML_UNAVAILABLE", "INFERENCE_FAILED"].includes(visit.adrPrediction.status) ? "UNAVAILABLE" : "SUCCESS") : "LOADING");
  ...
  const load = async ({ regenerate = false } = {}) => {
    ...
    try {
      ...
      if (result.status === "OUT_OF_VOCABULARY") { setState("NEEDS_INPUT"); setMessage(result.message || "One or more required inputs are outside the model vocabulary."); }
