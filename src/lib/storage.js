...
const write = (key, value) => { try { localStorage.setItem(key, JSON.stringify(value)) } catch { /* localStorage unavailable */ } }
...
export const getPatients = (doctorId) => getAllPatients().filter((patient) => patient.doctorId === doctorId)
export const getPatientById = (patientId) => getAllPatients().find((patient) => patient.id === patientId) || null
export const addPatient = (patient) => savePatients([...getAllPatients(), patient])
...
export const deletePatient = (patientId) => savePatients(getAllPatients().filter((patient) => patient.id !== patientId))
export const getNextPatientId = () => {
  ...
  } catch { return `P-${String(Date.now()).slice(-4)}` }
}
export const getTheme = () => { try { return localStorage.getItem('mrx_theme') === 'dark' ? 'dark' : 'light' } catch { return 'light' } }
export const setTheme = (theme) => { try { localStorage.setItem('mrx_theme', theme) } catch { /* localStorage unavailable */ } }
