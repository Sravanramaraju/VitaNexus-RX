...
const uiVisit = (item) => ({
  id: item.id, version: item.version, date: item.createdAt, prescribedDrug: item.prescription?.generic, prescription: { medicine: item.prescription ? { enteredName: item.prescription.enteredName, normalizedName: item.prescription.normalizedName, brand: item.prescription.brand, generic: item.prescription.generic, mappingSource: item.prescription.mappingSource, mappingVersion: item.prescription.mappingVersion } : null, dosage: item.prescription?.dosage || "", frequency: item.prescription?.frequency || "" }, indication: item.indication, indicationProvenance: item.indicationProvenance, diagnosis: item.indication, doctorNotes: item.latestNote?.text || "", riskResult: item.latestSafetyAssessment?.drugDrug || { severity: "NOT_EVALUATED", source: "DDInter 2.0" }, safetyResult: item.latestSafetyAssessment || null, recommendations: item.recommendations || [], adrPrediction: item.adrPrediction, eventProfile: item.eventProfile || null, feedback: item.followUps?.[0] || null, followUps: item.followUps || [], status: item.status, updatedRecommendations: item.recommendations || null,
});
...
export function PatientProvider({ children }) {
  ...
  const createPatient = async (basicInfo, clinicalInfo, consultationInfo) => {
    ...
    const consultation = await apiJson(`/patients/${patient.id}/consultations`, "POST", { indication: consultationInfo.indication, indicationId: consultationInfo.indicationSelection.id, indicationSource: consultationInfo.indicationSelection.source, indicationDatasetVersion: consultationInfo.indicationSelection.datasetVersion, candidateBrand: consultationInfo.prescription.medicine.brand || null, candidateGeneric: consultationInfo.prescription.medicine.generic || null, dosage: consultationInfo.prescription.dosage, frequency: consultationInfo.prescription.frequency });
    ...
    return { patient: uiPatient(patient), visit: { ...uiVisit(consultation), riskResult: safety.drugDrug, safetyResult: safety, recommendations: [] } };
  };
  const addVisitToPatient = async (patientId, consultationInfo) => {
    const consultation = await apiJson(`/patients/${patientId}/consultations`, "POST", { indication: consultationInfo.indication, indicationId: consultationInfo.indicationSelection.id, indicationSource: consultationInfo.indicationSelection.source, indicationDatasetVersion: consultationInfo.indicationSelection.datasetVersion, candidateBrand: consultationInfo.prescription.medicine.brand || null, candidateGeneric: consultationInfo.prescription.medicine.generic || null, dosage: consultationInfo.prescription.dosage, frequency: consultationInfo.prescription.frequency });
  ...
  const generateClinicalSafety = async (visitId) => { const result = await apiJson(`/consultations/${visitId}/clinical-safety-assessment`, "POST", {}); await refreshPatients(); return result; };
  const saveAdrPrediction = async (patientId, visitId) => { const result = await apiJson(`/consultations/${visitId}/adr-predictions`, "POST", {}); await refreshPatients(); return result; };
  const getAdrPrediction = async (visitId) => api(`/consultations/${visitId}/adr-predictions`);
  const correctConsultationIndication = async (visitId, selection, expectedVersion) => {
    const result = await apiJson(`/consultations/${visitId}/indication`, "PATCH", {
      indication: selection.display,
      indicationId: selection.id,
      indicationSource: selection.source,
      indicationDatasetVersion: selection.datasetVersion,
      expectedVersion,
    });
    await apiJson(`/consultations/${visitId}/clinical-safety-assessment`, "POST", {});
    await refreshPatients();
    return result;
  };
  const generateRecommendations = async (visitId) => { const result = await apiJson(`/consultations/${visitId}/recommendations`, "POST", {}); await refreshPatients(); return result; };
  ...
  return <PatientContext.Provider value={{ patients, refreshPatients, createPatient, addVisitToPatient, editPatient, deletePatientById, submitFeedback, saveFollowUpDraft, saveVisitNotes, generateClinicalSafety, saveAdrPrediction, getAdrPrediction, correctConsultationIndication, generateRecommendations, activeVisitId, setActiveVisitId }}>{children}</PatientContext.Provider>;
}
